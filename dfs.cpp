...
int *nDescendants; // ZY: number of descendants?
int *dfs;
int *adj;
int *adjAddress;
int *ear;
...
void DFS(int r)
{
   ...
   stack[0] = v;
   stackAddress[0] = adjAddress[v]; // stackAddress stores adj_list addresses of vertices (address of vertex v for example)
   ...
   {                            // set v to top vertex on stack
      ...
      for (int i = stackAddress[stack_pointer]; i < adjAddress[v + 1]; i++)
      {
         ...
         if (dfs[w] == -1) // if it's not yet visited, assign dfs number
         {
            ...
            stack[stack_pointer + 1] = w;
            stackAddress[stack_pointer + 1] = adjAddress[w];
            ...
            descend = 1;
            break; // this line breaks out of the loop so that adjacent nodes to v stop being searched and instead we descend to the children to explore them
         }
      ...
      if (descend)
      {
         stack_pointer++;
         continue;
      } // if the node has children to explore then descend to explore them
      ...
      if (v != r)
      { // r is root vertex, there is no such thing as parent[r], so we skip this line if v is the root r.
...
{
   return x == -1 || (dfs[q] < dfs[y]) || ((dfs[q] == dfs[y]) && (dfs[p] < dfs[x]) && !isAncestor(p, x)) || ((dfs[q] == dfs[y]) && isAncestor(x, p));
}
